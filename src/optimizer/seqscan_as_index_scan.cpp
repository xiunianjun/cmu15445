...
auto Optimizer::CheckIfColumnConstantConjunction(const AbstractExpressionRef &expr,
  ...
  if (const auto *logic_expr = dynamic_cast<const LogicExpression *>(expr.get());
    ...
    for (const auto &child : expr->GetChildren()) {
      bool res = CheckIfColumnConstantConjunction(child, column_indexes, first_values, last_values);
...
auto Optimizer::OptimizeSeqscanAsIndexScan(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::SeqScan) {
    ...
    if (seq_plan.Predicate() != nullptr &&
        CheckIfColumnConstantConjunction(seq_plan.Predicate(), &column_indexes, &first_values, &last_values)) {
      ...
      for (const auto *index : indices) {
        ...
        if (columns.size() == column_indexes.size()) {
          ...
          if (valid) {
            if (std::all_of(first_values.begin(), first_values.end(),
                            [](Value n) { return (n.CompareEquals(Value(TypeId::INTEGER, 0)) == CmpBool::CmpTrue); })) {
            ...
            if (std::all_of(last_values.begin(), last_values.end(),
                            [](Value n) { return (n.CompareEquals(Value(TypeId::INTEGER, 0)) == CmpBool::CmpTrue); })) {
              return std::make_shared<IndexScanPlanNode>(seq_plan.output_schema_, index->index_oid_,
                                                         seq_plan.Predicate());
            }
            ...
            return std::make_shared<IndexScanPlanNode>(seq_plan.output_schema_, index->index_oid_, seq_plan.Predicate(),
                                                       first_key, last_key);
          }
        }
      }
